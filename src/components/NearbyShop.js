...
// Import Leaflet awesome markers if available
...
const NearbyShop = () => {
    ...
    // Setup and update map when data is available
    useEffect(() => {
        ...
        mapInstanceRef.current = L.map(mapRef.current).setView([userLocation.lat, userLocation.long], 15);
        ...
        if (shops && shops.length > 0) {
            shops.forEach((shop) => {
                if (shop.shop_location && shop.shop_location.lat && shop.shop_location.long) {
                    const marker = L.marker([shop.shop_location.lat, shop.shop_location.long])
                        .addTo(mapInstanceRef.current)
                        .bindPopup(`<b>${shop.shop_name}</b><br>Distance: ${shop.distance} km`);
                    
                    marker.on('click', () => {
                        if (mapInstanceRef.current) {
                            mapInstanceRef.current.flyTo(marker.getLatLng(), 15, { animate: true, duration: 1 });
                            marker.openPopup();
                        }
                    });
                }
    ...
    // Handle shop view on map
    const handleViewOnMap = (shop) => {
        if (mapInstanceRef.current) {
            mapInstanceRef.current.flyTo([shop.shop_location.lat, shop.shop_location.long], 15, { animate: true, duration: 1 });
        }
    };
    ...
    const handleConfirmDetails = () => {
        navigate(`/confirmation?name=${medicineName}&quantity=${quantity}&price=${price}`);
    };
    ...
    return (
        <div>
            ...
            {/* Map */}
            <div id="map" ref={mapRef} style={{ height: '400px', marginBottom: '20px' }}></div>
            ...
            {showBuyPopup && (
                <div className="popup" style={{ display: 'block' }}>
                    <div className="popup-content">
                        <span className="close" onClick={() => setShowBuyPopup(false)}>&times;</span>
                        <h2>Buy Medicine</h2>
                        <div>
            ...
            {showDetailsPopup && (
                <div className="popup" style={{ display: 'block' }}>
                    <div className="popup-content">
                        <span className="close" onClick={() => setShowDetailsPopup(false)}>&times;</span>
                        <h2>Purchase Details</h2>
                        <div>
                            <p><strong>Medicine:</strong> {medicineName}</p>
                            <p><strong>Quantity:</strong> {quantity}</p>
                            <p><strong>Frequency:</strong> {frequency}</p>
                            <p><strong>Side Effects:</strong> {sideEffect}</p>
                            <p><strong>Precautions:</strong> {precaution}</p>
                        </div>
